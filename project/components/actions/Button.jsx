...
/** Button. Every button that starts work must take loading={true} until the work lands. Focus ring shows for keyboard focus only; press works for pointer, touch and Space/Enter. */
  ...
  return (
    <button type={type} disabled={disabled} aria-busy={loading || undefined} onClick={inert ? undefined : onClick}
      ...
      style={{ position: 'relative', display: fullWidth ? 'flex' : 'inline-flex', width: fullWidth ? '100%' : undefined, alignItems: 'center', justifyContent: 'center', gap: 4,
        ...
        transform: down && !reduced ? 'scale(var(--scale-press-button))' : 'scale(1)', transition: 'background var(--dur-default) var(--ease-default), transform var(--dur-default) var(--ease-default), box-shadow var(--dur-default) var(--ease-default)', ...style }} {...rest}>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, ...swap(!loading) }}>
