...
/** The One Link mark, a single link with two loops (assets/one-link-mark.svg), in brand blue on the surface, then the
    wordmark and the BETA badge. The white-on-blue tile is not drawn here: it is only for the app icon and the browser tab.
...
export function Logo({ product = 'One Link', showBeta = true, size = 'md', markOnly = false, style }) {
  const markH = size === 'sm' ? 18 : 20;
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, ...style }}>
      <svg viewBox="268 362 488 300" height={markH} width={Math.round(markH * 488 / 300)} style={{ flex: 'none', display: 'block' }}
        aria-hidden={markOnly ? undefined : true} role={markOnly ? 'img' : undefined} aria-label={markOnly ? product : undefined} focusable="false">
        <path fill="var(--content-accent-brand)" d={MARK_D} />
      </svg>
