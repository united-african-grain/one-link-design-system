...
/** Search field: 40px, radius 12, grouped-light; focus turns it white with a border and 4px halo. Shows a "/" keyboard hint (2px 4px, radius 4, #0000000d, body-4-strong secondary). Plain text input, so no native clear button. */
export function SearchField({ placeholder = 'Search trades, tickets, counterparties', value, onChange, hint = '/', maxWidth = 400, style, ...rest }) {
  ...
  return (
    <label onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{ display: 'flex', alignItems: 'center', gap: 8, height: 40, padding: '0 12px', borderRadius: 'var(--radius-sm)', background: bg, width: '100%', maxWidth, boxSizing: 'border-box', cursor: 'text',
      ...
      <input className="ol-search" type="text" role="searchbox" enterKeyHint="search" value={value} onChange={onChange} placeholder={placeholder} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
      ...
      {hint ? <kbd style={{ flex: 'none', padding: '2px 4px', borderRadius: 'var(--radius-4xs)', background: 'var(--hover-overlay-darker)', color: 'var(--content-secondary)', font: 'var(--font-body-4-strong)', letterSpacing: 'var(--body-4-tracking)', fontFeatureSettings: 'var(--features-text)' }}>{hint}</kbd> : null}
    </label>
  );
}
