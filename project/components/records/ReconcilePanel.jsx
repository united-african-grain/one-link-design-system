  ...
  return (
    <div style={{ background: 'var(--elevated)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-minimal-soft)', padding: 16, display: 'flex', flexDirection: 'column', gap: 16, ...style }}>
      <div ref={box} style={{ display: 'grid', gridTemplateColumns: stacked ? 'minmax(0,1fr)' : `repeat(${cols.length}, minmax(0,1fr))`, gap: 16 }}>
        {cols.map((c, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingLeft: i && !stacked ? 16 : 0, borderLeft: i && !stacked ? '1px solid var(--border-light)' : 0, paddingTop: i && stacked ? 16 : 0, borderTop: i && stacked ? '1px solid var(--border-light)' : 0, minWidth: 0 }}>
      ...
      {outcome === 'block' ? (
        <div style={{ borderRadius: 'var(--radius-md)', background: 'var(--error-subtle)', padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}><Button variant="outline" icon="rotate-ccw" onClick={onReweigh}>Re-weigh</Button><Button variant="outline" icon="camera" onClick={onPhotograph}>Photograph slip</Button></div>
        </div>
      ) : (
        <>
