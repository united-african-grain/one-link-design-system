...
The visual system borrows the craft of Polymarket's light theme (white-on-white elevation, translucent navy greys, physical 3D press buttons, spring motion) but none of its identity. By the owner's request the **BETA badge** and the **colour palettes** from that reference are kept. No other Polymarket marks, copy or motifs appear.

## Sources
- The One Link brief (pasted spec, §1–§8) — authoritative for every value here.
- Reference (orientation only): polymarket.us light theme; Inter (rsms.me/inter); Instrument Sans (Google Fonts); Lucide (lucide.dev); TradingView Lightweight Charts v5.
- A Claude artifact link was supplied (claude.ai/code/artifact/ce7a3a03-…) but could not be read; nothing from it is reflected here.
...
- Refs and IDs verbatim: `ZAM4702`, `GR10000377`, `T-0141`, `WBT10001605`. Meta lines joined with `·`. "You/your" addresses the user (`your position right now`). Portal copy is plainer still and never says "GRN" in a heading.
...
- **Type.** Inter variable for everything you read (480/540/580/600; features cv02 cv07 ss04 cv15 calt; headings add cv09 + opsz 14). Instrument Sans condensed (700, stretch 75%, uppercase, "case") for headline figures, commodity tabs, detail titles. Every figure `tabular-nums`. No monospace.
...
- **Motion.** `spring` (overshoot 1.015) for tab underline (300ms), label↔spinner swaps (500ms), toggles; `expand` for sheets/accordions; card hover cubic-bezier(.2,.75,.25,1) 400ms in / 160ms out; default 150ms ease-out. 3D press 50ms down / 100ms up. Buttons scale .97, cards .99. Live dot ping 1.5s. Reduced motion drops scale/translate/ping.
...
- **Focus.** 4px ring of the control's colour at 25%.
...
## Iconography
- **Lucide only**, loaded from CDN: `https://unpkg.com/lucide@0.460.0/dist/umd/lucide.min.js`, rendered via `components/core/Icon.jsx` (`<Icon name="triangle-alert" />`). No other set, no emoji, no Unicode glyphs as icons. Icons paint in currentColor.
...
- `components/` — React primitives, each with `.d.ts` and `.prompt.md`, one card per directory:
  - `core/` Icon, Text (+textStyle), Avatar (incl. commodity tile), Logo (with BETA)
  ...
  - `data/` Card + CardRow + Dot, QuantityChip, ShareBar, Figure (derivation popover), RailRow, DataTable (+RestrictedCell, DerivedCell, RefCell), ChartCard (+ChartAttribution)
...
- `ui_kits/one_link/` — desktop app (1440): Command Center (calm/exception), Trade Board (owner/restricted/loading, cards or table), Coverage — Wheat (local), Approval T-0141, Weighbridge tickets (default/silent/empty), GRN finalise (three outcomes), Grading dispute GR10000377, Stock board (Owner/Stock Control), Credit book. `Shell.jsx` holds Page / WithRail / PageHead / SectionLabel / FigureStrip.
...
- Lucide 0.460 UMD; TradingView Lightweight Charts 5.0.8 standalone.
